...
void LCDShield::figure(int type)
{
		LCDCommand(PASETP);
		LCDData(0);
		LCDData(131);
...
if(type)
	for(unsigned int i=0; i < (131*131*3)/2; i++)
		LCDData(fig1[i]);
else
for(unsigned int i=0; i < (131*131*3)/2; i++)
		LCDData(fig2[i]);
...
void LCDShield::start_refresh(void)
{
	LCDCommand(PASETP);
	LCDData(0);
	LCDData(131);
...
void LCDShield::print_two_pix_deep_color_8_to_4(uint8_t r1, uint8_t g1, uint8_t b1, uint8_t r2, uint8_t g2, uint8_t b2)
{
		LCDData((r1 & 0x00F0) | ((g1 >> 4) & 0x000F));
		LCDData((b1 & 0x00F0) | ((r2 >> 4) & 0x000F));
		LCDData((g2 & 0x00F0) | ((b2 >> 4) & 0x000F));
}
